...
// Split ouput file as 
/*--------------------------------------------------------------------------------*/
void SusyHistos::SaveSplitWHHistograms(TDirectory* _hDir,
				       int method,
				       bool mcMll, bool isZAlpgenSherpa, 
				       string sys1, string sys2)
{
...
// Split ouput file as 
/*--------------------------------------------------------------------------------*/
void SusyHistos::SaveSplit3LHistograms(TDirectory* _hDir,
				       int method,
				       bool mcMll, bool isZAlpgenSherpa, 
				       string sys1, string sys2)
{
  ...
  TFile *histoFile[ML_NSR][ML_N];
  std::map<string, std::pair< int,int> > _label;  //eg: CR2LepOS_EE

  //Put  each SR/Lep combo in a dir otherwise too many files to ls
  for(int ilep=0; ilep<ML_N; ilep++){	
    for(int isr=0; isr<ML_NSR; isr++){ //SR/CR's loop
      string chanSR =  "ML_" + string(ML_SRNAME[isr]) + "_" + string(ML_FLAV[ilep]);
      //cout << "Label " << chanSR << endl;
      _label.insert(_label.end(),std::make_pair(string(chanSR), std::make_pair(ilep,isr)));
      string dir =  baseDir + chanSR; 
      gSystem->mkdir(dir.c_str(),kTRUE);
  ...
  while((obj=next())) { 
    ...
    string _name =  string(obj->GetName());
    string _name2 = _name.substr(3); // ML_
    uint idx1 = _name.find("_"); //idx of 1st _
    uint idx2 = _name.find("_",idx1+1); //idx of 2nd _
    uint idx3 = _name.find("_",idx2+1); //idx of 2nd _
    string _subName = _name.substr(0,idx3);
    //cout << "histName " << _name <<  "\t histo name extracted " << _subName << "*" << endl;
  ...
  for(int ilep=0; ilep<ML_N; ilep++){	
    ...
    cout << "Histograms for " << ML_FLAV[ilep] << " "  << count << " files saved " << endl;
  }
